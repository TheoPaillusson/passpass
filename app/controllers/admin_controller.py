...
def edit_question(quiz_id, question_id):
    ...
    if request.method == 'POST':
        ...
        question.correction = form.correction.data

        # Handle the question image update
        ...
        for key in request.form:
            if key.startswith("sub_questions-") and key.endswith("-question_statement"):
                ...
                print(f"Subquestion ID: {sub_id}, Correction: '{correction}'")
        ...
        for key in request.form:
            if key.startswith("sub_questions-") and key.endswith("-id"):
                submitted_sub_ids.add(request.form[key])

        for sub_id, sub in existing_sub_ids.items():
            if not sub_id or sub_id not in submitted_sub_ids:
                db.session.delete(sub)


        print("Submitted subquestion IDs:", submitted_sub_ids)
        print("Existing subquestion IDs:", list(existing_sub_ids.keys()))   
        db.session.commit()
